window.onload = async () => {
    ...
    const info = peliculas.info;

    $id.value = info.id;
    $title.value = info.title;
    $rating.value = info.rating;
    $awards.value = info.awards;
    $release_date.value = new Date(info.release_date).toISOString().slice(0, 10);
    ...
    $edit.addEventListener("click", async () => {
          ...
          const response = await fetch(`http://localhost:3031/api/movies/update/${info.id}`, {
                method: "PUT",
                headers: {
